...
var letter_display_timer := 0.10
var space_display_timer := 0.08
var punctuaction_display_timer := 0.5
...
func display_text(text_to_display: String): 
	...
	if size.x > MAX_WIDTH:
		...
		await resized #aguardar redimencionamento em x
		await resized #aguardar redimencionamento em y
		custom_minimum_size.y = size.y
		
	global_position.x -= size.x / 2 
	global_position.y -= size.y + 24 / 2
	text_label.text = ""
	display_letter()
